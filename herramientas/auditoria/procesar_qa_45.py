...
procesar_qa_45.py
-------------------
Aplica el resultado del QA visual de los 45 candidatos pendientes:
  - "ok"        -> registra en productos.json (imagen -> archivo candidato) y
                   marca "corregida" en el CSV.
  - "sospechosa"-> mueve el archivo candidato a reales/_descartadas_qa2/ (no
                   lo borra) y deja el producto en "sin_resolver" con nota.
"""
...
RESULTADO_TXT = RAIZ / "reportes" / "qa_45_resultado.txt"

from descargar_imagenes_serper import generic_for

resultados = {}
for linea in RESULTADO_TXT.read_text(encoding="utf-8").splitlines():
    linea = linea.strip()
    if not linea:
        continue
...
for fila in filas:
    ...
    if sku in ok_ids:
        ...
        nota = "Fase 2 (dominios ampliados, ronda pre-nuevos-dominios): reemplazada y confirmada por QA visual"
        ...
        nota = "QA visual (candidato pre-nuevos-dominios): sigue sin coincidir, va a cola de reintento con nuevas fuentes"
        fila["notas"] = f"{fila['notas']}; {nota}" if fila["notas"] else nota
